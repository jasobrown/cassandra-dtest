...
current_2_0_x = VersionMeta(name='current_2_0_x', family='2.0.x', variant='current', version='2.0.17', min_proto_v=1,
                            max_proto_v=2, java_versions=(7,))

indev_2_1_x = VersionMeta(name='indev_2_1_x', family='2.1.x', variant='indev', version='github:apache/cassandra-2.1',
                          min_proto_v=1, max_proto_v=3, java_versions=(7, 8))
current_2_1_x = VersionMeta(name='current_2_1_x', family='2.1.x', variant='current', version='2.1.17', min_proto_v=1,
                            max_proto_v=3, java_versions=(7, 8))

indev_2_2_x = VersionMeta(name='indev_2_2_x', family='2.2.x', variant='indev', version='github:apache/cassandra-2.2',
                          min_proto_v=1, max_proto_v=4, java_versions=(7, 8))
current_2_2_x = VersionMeta(name='current_2_2_x', family='2.2.x', variant='current', version='2.2.9', min_proto_v=1,
                            max_proto_v=4, java_versions=(7, 8))

indev_3_0_x = VersionMeta(name='indev_3_0_x', family='3.0.x', variant='indev', version='github:apache/cassandra-3.0',
                          min_proto_v=3, max_proto_v=4, java_versions=(8,))
current_3_0_x = VersionMeta(name='current_3_0_x', family='3.0.x', variant='current', version='3.0.16', min_proto_v=3,
                            max_proto_v=4, java_versions=(8,))

indev_3_x = VersionMeta(name='indev_3_x', family='3.x', variant='indev', version='github:apache/cassandra-3.11',
                        min_proto_v=3, max_proto_v=4, java_versions=(8,))
current_3_x = VersionMeta(name='current_3_x', family='3.x', variant='current', version='3.11.2', min_proto_v=3,
                          max_proto_v=4, java_versions=(8,))

indev_trunk = VersionMeta(name='indev_trunk', family='trunk', variant='indev', version='github:apache/trunk',
                          min_proto_v=3, max_proto_v=5, java_versions=(8,))


# MANIFEST maps a VersionMeta representing a branch/variant to a list of other VersionMetas, that are supported upgrades
# Note on versions: 2.0 must upgrade to 2.1. Once at 2.1 or newer, upgrade is supported to any later version,
# including trunk (for now).
# "supported upgrade" means a few basic things, for an upgrade of version 'A' to higher version 'B':
#   1) The cluster will function in a mixed-version state, with some nodes on version A and some nodes on version B.
#      Schema modifications are not supported on mixed-version clusters.
#   2) Features exclusive to version B may not work until all nodes are running version B.
#   3) Nodes upgraded to version B can read data stored by the predecessor version A, and from a data standpoint will
#      function the same as if they always ran version B.
#   4) If a new sstable format is present in version B, writes will occur in that format after upgrade.
#      Running sstableupgrade on version B will proactively convert version A sstables to version B.
MANIFEST = {
...
# 1) Add a VersionMeta for each version you wish to test (see examples below). Update the name, family, version, and
# protocol restrictions as needed. Use a unique name for each VersionMeta.
# 2) Update OVERRIDE_MANIFEST (see example below).
# 3) If you want to test using local code, set the version attribute using local slugs in the format
# 'local:/path/to/cassandra/:branch_name'
# 4) Run the tests!
#      To run all, use 'pytest --execute-upgrade-tests'.
#      To run specific tests, use 'run_dtests --dtest-print-tests-only' to preview the test names,
#      then run pytest using the desired test name.
#      Note that pytest outputs test names in a format that needs to be tweaked a bit
#      before they will run from the command line.
custom_1 = VersionMeta(name='custom_branch_1', family='2.1.x', variant='indev', version='local:some_branch',
                       min_proto_v=3, max_proto_v=4, java_versions=(7, 8))
custom_2 = VersionMeta(name='custom_branch_2', family='2.2.x', variant='indev', version='git:trunk',
                       min_proto_v=3, max_proto_v=4, java_versions=(7, 8))
custom_3 = VersionMeta(name='custom_branch_3', family='3.0.x', variant='indev', version='git:cassandra-3.5',
                       min_proto_v=3, max_proto_v=4, java_versions=(7, 8))
custom_4 = VersionMeta(name='custom_branch_4', family='3.x', variant='indev', version='git:cassandra-3.11',
                       min_proto_v=3, max_proto_v=4, java_versions=(7, 8))
custom_jeb = VersionMeta(name='custom_JEB', family ='trunk', variant='indev', version='git:trunk',
                         min_proto_v=3, max_proto_v=5, java_versions=(8,))
...
def build_upgrade_pairs():
    ...
    for origin_meta, destination_metas in list(manifest.items()):
        for destination_meta in destination_metas:
            # None means we don't care about a version, which means we don't care about iterations involving it either
            if not (origin_meta and destination_meta):
                logger.debug("skipping class creation as a version is undefined (this is normal), versions: {} and {}"
                             .format(origin_meta, destination_meta))
                continue

            if not _is_targeted_variant_combo(origin_meta, destination_meta):
                logger.debug("skipping class creation, no testing of '{}' to '{}' (for {} upgrade to {})"
                             .format(origin_meta.variant, destination_meta.variant,
                                     origin_meta.name, destination_meta.name))
                continue

            if not _have_common_proto(origin_meta, destination_meta):
                logger.debug("skipping class creation, no compatible protocol version between {} and {}"
                             .format(origin_meta.name, destination_meta.name))
                continue
